  ...
  <div class="app-container">
    <el-form ref="form" v-loading="formLoading" :model="form" label-width="100px" :rules="rules">
      ...
      <el-form-item label="填空答案：" required>
        <el-form-item v-for="item in form.items" :key="item.prefix" :label="item.prefix" label-width="50px" class="question-item-label">
          <el-input v-model="item.content" class="question-item-content-input" style="width: 80%" @focus="inputClick(item,'content')" />
          <span class="question-item-span">分数：</span><el-input-number v-model="item.score" :precision="1" :step="1" :max="100" />
        </el-form-item>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    questionItemReset(content) {
      const spanRegex = new RegExp('<span class="gapfilling-span (.*?)">(.*?)<\\/span>', 'g')
      const _this = this
      const newFormItem = []
